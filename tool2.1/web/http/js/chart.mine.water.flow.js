...
//			}
	...
	var UI = (function(){
		var margin = 8;
		var isOnResizeFlag = false;
		var CurrentTotalComnNums;
		 ...
		 * 瀑布流模式
		 */
		var showWarterFlow = function(){
			var container = document.getElementById('my-gallery-thumbnail');
			if(!container)
				return ;

			var boxes = $(container).find(".col-sm-6.col-md-3");;
			...
			for (var i = 0,l= parseInt(boxes.length); i <l ; i++) {
				if((boxes[i].offsetWidth+margin) != boxWidth)
		 ...
		 * @returns {*}
		 */
		var getMinBoxHeightFromColumnHeight = function(heightAarry){
			var minBoxHeightIndex = 0;
			var minHeight = heightAarry[minBoxHeightIndex];
		 ...
		 * @returns {*}
		 */
		var getMaxBoxHeightFromColumnHeight = function(heightAarry){
