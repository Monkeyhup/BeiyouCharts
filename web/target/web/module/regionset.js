...
define(function (require, exports, module) {
    ...
    var mustache = require("mustache");                    //是模块？渲染与替换的作用  替换大括号内的内容？
    var SpatialQuery = require("spatial.query");           //具体作用？
    var regPanel = $("#region-panel");                     //区划设置弹出面板   这是mobile端？
    var defaultRegions = [],
        currRegions,
        selRegions = [],
        currLevel,
    ...
    var init = (function () {
         ...
         * 热度区域初始化
         */
        var hotRegionInit = function (regions) {                              //有两个hotRegionInit

            var obj = {"hotRegions":regions};                                 //hotRegions是什么
    ...
    var show = function () {
        ...
        init();
    };           //其中regPanel的属性都是mobile端的？region-tool i.down.icon是什么
    ...
    var drillAction = function (_region) {
        ...
        if(level<6){  //村级别就不往下走了
            ...
            events.trigger("region.switch",selRegions); //触发行政区划改变事件   //switch在哪里
            RegionTree.location(); //定位
        ...
        var linkage = function (visiable) {
            ...
            if (idstr) {
                ...
                for (var i = 0, len = ids.length; i < len; i++) {
                    tree.setCheck(ids[i], false);//去掉选中          //setCheck什么意思
                }
                listRegions();
            }
            if (currRegions && tree) {
...
//                }
        ...
        var location = function (_regions, callback) {
            ...
            SpatialQuery.queryRegionFeature(_regions,1, function (feas) {               //查询地图feature要素
