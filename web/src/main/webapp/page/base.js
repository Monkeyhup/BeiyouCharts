...
SGIS.Region.getPrefixCode = function (regionCode) {
    ...
    if (code.indexOf("#") != -1) {
        //#代表全部的意思
        return code.substring(0, code.indexOf("#"));      //#是什么意思
    } else {
        if (code.length == 12) {
            if (code.lastIndexOf("000000000000") == 0) {
                re = code.substring(0, 0); //国家,12个0             //00?
...
(function () {
    ...
    var WEB_NAME = "/visual/";
    var BASE_NAME="/base2.1/";                          			//base项目名称
    var GALLERY_NAME="/gallery2.1/";                   			//gallery项目名称
    var TOOL_NAME="/tool2.1/";                        			//tool项目名称
    var WEB_NAME="/web/";                                           //web项目名称
    ...
    var Req = function (api) {
        ...
        this._debug = false;
        if(api=="user/is/login"){
            this.serverName=GALLERY_NAME;//登陆用服务名
        }else{
