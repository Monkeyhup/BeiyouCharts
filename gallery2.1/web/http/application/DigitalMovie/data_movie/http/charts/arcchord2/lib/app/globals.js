 ...
 */
var maxWidth=Math.max(400,Math.min(screen.height,screen.width)-250);
//var maxWidth=Math.max(600,Math.min(screen.height,screen.width)-250);

var outerRadius = (maxWidth / 2),                       //外半径
    innerRadius = outerRadius - 100 -100,                    //内半径
...
var e_fill= d3.scale.ordinal().range(["#2C8DCB","#39BFF9","#39BFF9","#39BFF9","#39BFF9"]);   //输出弦填充颜色随机
var i_fill= d3.scale.ordinal().range(["#EFCE0D","#D45E26","#FFFF00","#F0DE94","#FAB936"]);      //输入弦填充颜色随机
...
dGroup.append("text")                       //年份
    .attr("id","midLabel")
    .attr("class","midLabel")
    .attr("transform", "translate(" + (outerRadius - 20) + ","  + (outerRadius + 40) +")")
    .style("font-size","16px");
