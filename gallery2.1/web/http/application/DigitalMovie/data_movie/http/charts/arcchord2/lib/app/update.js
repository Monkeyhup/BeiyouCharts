...
function update(y) {
    ...
    //var n_i_labels = [];
    //for(var ii= 0,iiLen = i_labels.length;ii<iiLen;ii++){
    //    if(i_labels[ii].label == null || i_labels[ii].label == "null"){
    //        continue;
    //    }
    //    n_i_labels.push(i_labels[ii]);
    //}
    //i_labels = n_i_labels;
    ...
    var fs=15;
    //console.log("fs="+fs);
    tI=formatCurrency(tI);
    tX=formatCurrency(tX);
    //console.log("tI="+tI);
    //console.log("tX="+tX);
    //console.log("年份="+(baseYear+year));
    dGroup.transition()
        .select("text#mainLabel")
        .delay(delay)
        .text("文化创意产业增加值（亿元）:"+tI)
        .attr("transform", "translate(" + (outerRadius-60)  + ","  + (outerRadius + 50) +")")
        .style("font-size", fs + "px");

    dGroup.transition()
        .select("text#midLabel")
        .delay(delay)
        .text(baseYear+year+1)
        .attr("transform", "translate(" + (outerRadius-10)  + ","  + (outerRadius + 70) +")")
        .style("font-size", fs + "px");
    ...
    dGroup.transition()
        ...
        .delay(delay)
        .text("文化创意产业增幅（%）："+tX)
        .attr("transform", "translate(" + (outerRadius-60) + ","  + (outerRadius + 90) +")")
        .style("font-size", fs + "px");
     ...
     * 隐藏null
     */
    $("#svg .imports").find("text.import").each(function(i,o){
        var $o = $(o);
        var html = $o.html();

        if(html.indexOf("null") > -1){
            $(o).css({"display":"none"});
        }
    });
    $("#svg .exports").find("text.export").each(function(i,o){
        var $o = $(o);
        var html = $o.html();
