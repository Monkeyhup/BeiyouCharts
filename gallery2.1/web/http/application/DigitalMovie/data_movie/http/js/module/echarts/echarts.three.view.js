...
define(function(require, exports, module) {
	...
	EChartMap.prototype = {
		init:function(config){
			...
			//放置当前的颜色(该颜色是点击饼是放入的，其他的没有颜色)
			var isClickPieInit = false;	//是否是点击饼进来的
			if(typeof config.isSetDataRangeColors == "boolean" && config.isSetDataRangeColors === true){
				that.option.dataRange.color = config.dataRangeColors;
				isClickPieInit = true;
			}else{
				delete that.option.dataRange.color;
			}
			...
			if(myChart){
				this.getOptionFromConfig(config,function(){
					...
					if(config.option){
						if(isClickPieInit && config.option.dataRange && config.option.dataRange.color){
							//如果是点击饼，直接使用固定的颜色（由饼图传来的颜色）
							delete config.option.dataRange.color;
						}
		...
		setAreaGroupColor:function(dataGroup,colors){
			...
			if(dataGroup && colors && dataGroup.length == colors.length){
				var splitList = [];
				var isHas = {};		//记录所有出现的数字（禁止出现重复）
				for(var i= 0;i<dataGroup.length;i++){
					for(var j=0;j<dataGroup[i].length;j++){
						var item = dataGroup[i][j].value;

						if(isHas["has_"+item] &&　isHas["has_"+item] != ""){
							item = item + Math.random()*10;
						}
						isHas["has_"+item] = "has_"+item;
						splitList.push({
							start: item,
							end: item,
							label: dataGroup[i][j].name,
							color: colors[i]
						});
					}
				}
				//指定区域颜色
				that.option.dataRange.splitList = splitList;
			}else{
				//移除区域颜色
				delete that.option.dataRange.splitList;
			}
	...
	var EChartPieSingle = function(){
		...
		this.colors = ["#EF9359","#E8C374","#A4BBDA","#70B5AA",	
				...
				if(re && re != null){
							...
							if(param){
								...
								for ( var i = 0; i < data.length; i++) {
									if(data[i].name == param.name){
										...
										if(currentColor){
											//添加颜色
											mapConfig1.isSetDataRangeColors = true;
											mapConfig1.dataRangeColors = [currentColor,"#ffffff"];
										}
					...
					that.pieSingle.init(pieConfig,function(pie){
						...
						var mapConfig = $.extend({},config,{
							...
							isShowLegend:false,
							isShowRoamController:false,
							isShowDataRangeController:false
						});
						mapConfig.url = null;
