	...
	BarDrill.prototype = {
		...
		excute: function(callback) {
			...
			function showData(re){
				...
				svg.selectAll(".x.axis").selectAll(".tick>text").attr("y","0");
				
				//重置高度
				resetHeight();
				
				//自定义下钻
				var selfDrillDown = function(){
					x.domain([0, root.children[0].value]).nice();
					down(root.children[0], 0);
				}
				
				//自定义上钻
				var selfDrillUp = function(){
					x.domain([0, root.value]).nice();
					down(root, 0);
				}
				
				//自动下钻第一个
				//setTimeout(selfDrillDown,3000);
	
				callback && callback(selfDrillDown,selfDrillUp);
			}
			...
			function up(d) {
				...
				exitTransition.select("rect").attr("width12", function(d) {
					return x(d.value);
				}).style("fill", z(true));
