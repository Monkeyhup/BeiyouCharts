define(function(require, exports, module) {
	...
	BarIce.prototype = {
		...
		excute: function() {
			...
			var vis = d3.select(containerId).append("div").attr("id", "chart_ice_" + random).attr("class", "chart_" + random).style("width", w + "px").style("height", h + "px").append("svg:svg").attr("width", w).attr("height", h);
			...
			function showData(root){
				...
				//放入显示的文字
				g.append("svg:text").attr("class","text-name").attr("transform", function(d){
					return "translate(14," + (d.dx * ky / 2-12*0.9+10) + ")";
				}).attr("dy", ".35em").style("font-size","1.8em").style("opacity", function(d) {
					return d.dx * ky > 20 ? 1 : 0		//12->20
				}).text(function(d) {
					//获取单位
					var unit = getUnit(d.name);
					
					if(unit && unit != ""){
						var name = d.name.replace(unit,"");
						nameValue[d.name] = unit;
						return name;
					}else{
						nameValue[name] = "";
						return d.name;
				...
				//文字值
				g.append("svg:text").attr("class","text-value").attr("transform", function(d){
					return "translate(8," + (d.dx * ky / 2+12*0.9+20) + ")";
				}).attr("dy", ".35em").style("font-size","1.6em").style("opacity", function(d) {
					return d.dx * ky > 60 ? 1 : 0		//12->20
				}).text(function(d) {
					return nameValue[d.name] ? nameValue[d.name] : d.name;
				...
				function click(d) {
					...
					t.select("text.text-name").attr("transform", function(d){
						return "translate(14," + (d.dx * ky / 2-12*0.9+10) + ")";
					}).style("opacity", function(d) {
						return d.dx * ky > 20 ? 1 : 0		//12->20
					});
					
					t.select("text.text-value").attr("transform", function(d){
						return "translate(8," + (d.dx * ky / 2+12*0.9+20) + ")";
					}).style("opacity", function(d) {
						return d.dx * ky > 60 ? 1 : 0		//12->20
					});
				...
				function getUnit(name){
					var leftC = "(",rightC = ")",leftC2 = "（",rightC2 = "）";
					
					if(name.indexOf(leftC) > -1 && name.indexOf(rightC) > name.indexOf(leftC)){
						return name.substr(name.indexOf(leftC),name.indexOf(rightC));
					}
					
					if(name.indexOf(leftC2) > -1 && name.indexOf(rightC2) > name.indexOf(leftC2)){
						return name.substr(name.indexOf(leftC2),name.indexOf(rightC2));
					}
